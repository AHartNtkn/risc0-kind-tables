...
2. Publish `anoma-risc0-kind-tables` from the `main` merge commit: `just crates-publish --dry-run`, then `just crates-publish`. Publishing requires every git-pinned dependency to be replaced by a registry release first.
